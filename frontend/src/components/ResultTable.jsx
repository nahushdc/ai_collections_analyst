        ...
        <thead>
          <tr className="border-b border-slate-700/50 bg-slate-800/80">
            {columns.map((col) => (
              <th
                key={col}
                className="px-2.5 py-2 text-left text-[10px] font-semibold uppercase tracking-widest text-slate-400 sm:px-4 sm:py-3 sm:text-xs"
              >
                {col}
              </th>
            ))}
            ...
            return (
              ...
              >
                {columns.map((col) => (
                  <td
                    key={col}
                    className={`px-2.5 py-2 whitespace-nowrap sm:px-4 sm:py-3 ${grandTotal ? 'text-slate-100 font-semibold' : getCellClass(col)}`}
                  >
